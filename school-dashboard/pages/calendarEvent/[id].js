...
export default function CalendarEvent({query, data: calendar}) {
    ...
    const eventDayOfTheWeek = new Date(calendar.date).toLocaleDateString('en-US', { weekday: 'long' });
    const eventHasLink = calendar.link ? true : false;
    const linkWithHTTP = calendar.link.startsWith('http') ? calendar.link : `http://${calendar.link}`;
    const eventLinkTitle = calendar.linkTitle ? `Link: ${calendar.linkTitle}` : 'Attached Link';
  return (
    <CalendarStyles>
        <h1>{calendar.name}</h1>
        <p>{calendar.description}</p>
        <p>{eventDayOfTheWeek} - {date}</p>
        <p>{daysUntilString} {daysUntilStringPlural} until event</p> 
        {eventHasLink && <a href={linkWithHTTP}>{eventLinkTitle}</a>}
        <br />
        <p>Event is for Students or Teachers: {calendar.status}</p>
        <p>Event was created by: {calendar.author?.name} on {dateCreated}</p>
    </CalendarStyles>
  )
}

const GET_SINGLE_CALENDAR_EVENT = `
...
export async function getServerSideProps(context) {
    ...
    const headers = {
      ...
      const fetchThisCalendar = async () => graphQLClient.request(GET_SINGLE_CALENDAR_EVENT, {id: context.query.id});
